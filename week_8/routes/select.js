...
router.get('/', async function(req, res) { // '/select' 를 '/' 로 축약해서 표현
    const employee = await selectSql.getEmployee(); // employee 에 getemployee 함수 값 저장
    const department = await selectSql.getDepartment(); // department 에 getdepartment 함수 값 저장
    ...
    res.render('select', { // select.hbs 파일을 불러오기
        ...
        employee,
        department
    }); // title, title2, employee, department 데이터 값 념겨주기
});
